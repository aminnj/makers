...
for uaf in range(3,11):
    ...
    # print "#%02x%02x%02x" % tuple(c1+f*(c2-c1))
    # print "<span style='border: 2px solid #%02x%02x%02x;'> uaf-%i </span> &nbsp;" % tuple(list(c1+f*(c2-c1))+[uaf])
    buff += "<span style='border: 2px solid #%02x%02x%02x;'> uaf-%i </span> &nbsp; " % tuple(list(c1+f*(c2-c1))+[uaf])


with open("overview.html.replace","r") as fhin:
    data = fhin.read().replace("UAFPLACEHOLDER",buff)

with open("overview.html","w") as fhout:
    fhout.write(data)
